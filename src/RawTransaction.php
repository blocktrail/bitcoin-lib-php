...
class RawTransaction
{
    ...
    {
        // $encoded_version - get little endian
        $encoded_version = $bytes = self::_dec_to_bytes($raw_transaction_array['version'], 4, true);
        ...
        $encoded_inputs = self::_encode_vint($decimal_inputs_count);
        if ($decimal_inputs_count > 0) {
            $encoded_inputs .= self::_encode_inputs($raw_transaction_array['vin'], $decimal_inputs_count);
        }

        // $encoded_outputs - set varint, then work out if output hex is required.
        $decimal_outputs_count = count($raw_transaction_array['vout']);
        $encoded_outputs = self::_encode_vint($decimal_outputs_count);
        if ($decimal_outputs_count > 0) {
            $encoded_outputs .= self::_encode_outputs($raw_transaction_array['vout'], $decimal_outputs_count);
        }
    ...
    public static function _create_txin_signature_hash($raw_transaction, $json_inputs, $specific_input = -1, $e = null)
    {
        ...
        if ($specific_input == -1) {
            ...
            foreach ($decode['vin'] as $vin => $input) {
                ...
                if (isset($inputs[$vin]->redeemScript)) {
                    $copy['vin'][$vin]['scriptSig']['hex'] = $inputs[$vin]->redeemScript;
                } else {
                    $copy['vin'][$vin]['scriptSig']['hex'] = $inputs[$vin]->scriptPubKey;
                }
            ...
            $copy = $decode;
            if (isset($inputs[$specific_input]->redeemScript)) {
                $copy['vin'][$specific_input]['scriptSig']['hex'] = $inputs[$specific_input]->redeemScript;
            } else {
                $copy['vin'][$specific_input]['scriptSig']['hex'] = $inputs[$specific_input]->scriptPubKey;
            }
            $hash = hash('sha256', hash('sha256', pack("H*", self::encode($copy) . $sighashcode), true));
        }
        return $hash;
    ...
    public static function decode_redeem_script($redeem_script, $data = array())
    {
        ...
        if (!isset($data['m']) || count($data) == 0) {
            ...
            } elseif (in_array(gmp_cmp(gmp_init($next_op, 16), gmp_init('51', 16)), array('0', '1'))
    ...
    public static function sign($wallet, $raw_transaction, $inputs, $magic_byte = null, $magic_p2sh_byte = null)
    {
            ...
            if (isset($wallet[$tx_info['hash160']])) {
                ...
                if ($key_info['type'] == 'scripthash') {
                    ...
                    foreach ($key_info['keys'] as $key) {
                        $point = new Point(
                            $generator->getCurve(),
                            gmp_init(substr($key['uncompressed_key'], 2, 64), 16),
                            gmp_init(substr($key['uncompressed_key'], 66, 64), 16),
                            $generator->getOrder()
                        );

                        $_public_key = new PublicKey($generator, $point);
                        $_private_key = new PrivateKey($_public_key, gmp_init($key['private_key'], 16));
                        $sign = $_private_key->sign(
                            gmp_init($message_hash[$vin], 16),
                            gmp_init((string)bin2hex(openssl_random_pseudo_bytes(32)), 16)
                        );
                    ...
                    $decode['vin'][$vin]['scriptSig']['hex'] =
                        self::_apply_sig_scripthash_multisig($signatures, $key_info);
                ...
                if ($key_info['type'] == 'pubkeyhash') {
                    ...
                    $point = new Point(
                        $generator->getCurve(),
                        gmp_init(substr($key_info['uncompressed_key'], 2, 64), 16),
                        gmp_init(substr($key_info['uncompressed_key'], 66, 64), 16),
                        $generator->getOrder()
                    );
                    ...
                    if ($sign !== false) {
                        $sign_count++;
                        $decode['vin'][$vin]['scriptSig']['hex'] =
                            self::_apply_sig_pubkeyhash(self::encode_signature($sign), $key_info['public_key']);
                    }
                    $req_sigs++;
                }
            } else {
        ...
        // If it's fully signed, perform signature verification
        //  return true if valid, or invalid if signatures are incorrect.
        if (($req_sigs - $sign_count) <= 0) {
            $complete = (self::validate_signed_transaction($new_raw, $inputs, $magic_byte, $magic_p2sh_byte) == true) ? 'true' : 'false';
        } else {
            $complete = 'false';
        }
